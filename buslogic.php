...
class clsfil
{
    ...
    public function dspshrfil($cod)
    {
        $con=new clscon();
        $qry="call dspshrfil('$cod')";
        $res=  mysql_query($qry) or die(mysql_error());
        $objarr=array();
        $i=0;
        while($r= mysql_fetch_array($res))
...
class clsgrpmem
{
    ...
    {
        $con=new clscon();
        $qry="call updgrpmem('$this->grpmemcod','$this->grpmemgrpcod','$this->grpmemregcod')";
        $res=  mysql_query($qry);
        $d=  mysql_affected_rows();
        mysql_close();
        if($d==1)
    ...
    {
       $con=new clscon();
       $qry="call fndgrp('$this->grpmemcod')";
       $res=  mysql_query($qry);
       if($r=  mysql_fetch_array($res))
       {
           $this->grpmemcod=$r[0];
...
class clsseldoc
{
    ...
    {
        $con=new clscon();
        $qry="call insseldoc('$this->seldocregcod','$this->seldoctit','$this->seldocdsc','$this->seldocfil','$this->seldocprc','$this->seldoccatcod',@cod)";
        $res=  mysql_query($qry)or die(mysql_error());
        $res1=  mysql_query("select @cod");
        $r=  mysql_fetch_row($res1);
        return $r[0];
    ...
    public function dspseldocs($ccod)
    {
        $con=new clscon();
        $qry="call dspseldocs('$ccod')";
        $res=  mysql_query($qry) or die(mysql_error());
        $objarr=array();
        $i=0;
        while($r= mysql_fetch_row($res))
    ...
    {
       $con=new clscon();
       $qry="call fndseldoc('$this->seldoccod')";
       $res=  mysql_query($qry);
       if($r=  mysql_fetch_array($res))
       {
           $this->seldoccod=$r[0];
